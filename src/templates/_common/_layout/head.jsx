...
const Head = () => (
  <head>
    ...
    <meta
      name="description"
      content={` ${it.L(
        "[_1] gives everyone an easy way to participate in the financial markets. Trade with as little as $1 USD on major currencies, stock indices, commodities, and synthetic indices.",
        it.broker_name
      )}`}
    />
    <meta
      name="keywords"
      content={` ${it.L(
        "binary options, forex, forex trading, online trading, financial trading, binary trading, index trading, trading stock indices, forex trades, trading commodities, binary options strategy, binary broker, binary bet, binary options trading platform, binary strategy, finance, investment, trading"
      )}`}
    />
    ...
    <meta
      ...
      content="roReCEK-wNa1EMA6ZM9a4zCOQOMqxfJjvfsZMC9qh_k"
    />
    <meta property="og:title" content={it.broker_name} />
    <meta property="og:type" content="website" />
    <meta
      property="og:image"
      content={it.url_for("images/common/og_image.gif")}
    />
    ...
    <script
      ...
      src={`${it.root_url}pushwoosh-web-notifications.js`}
      async
    />

    <Favicons />

    {it.languages.map((lang, inx) => (
      <link
        key={inx}
        rel="alternate"
